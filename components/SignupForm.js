...
export default function SignupForm({ open, onClose, onLogin }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40 animate-fade-in">
      <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-md relative animate-fade-in-up">
        ...
        <h2 className="text-2xl font-extrabold text-blue-900 mb-6 text-center">Sign Up for MieT</h2>
        <form className="space-y-4">
          <input type="text" placeholder="Full Name" value={name} onChange={e => setName(e.target.value)} className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-400" />
          ...
          <button type="submit" className="w-full py-2 bg-blue-600 text-white font-bold rounded-lg shadow hover:bg-blue-700 transition">Sign Up</button>
        </form>
        ...
        <div className="flex gap-4 justify-center mb-4">
          <button className="p-2 rounded-full border hover:bg-blue-50 transition" aria-label="Signup with Google">
          ...
          <button className="p-2 rounded-full border hover:bg-blue-50 transition" aria-label="Signup with Facebook">
        ...
        <div className="text-center text-sm text-gray-500">Already have an account? <button className="text-blue-600 font-bold hover:underline" onClick={onLogin}>Log In</button></div>
      </div>
    </div>
  );
}
