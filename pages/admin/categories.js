...
function CategoriesPage() {
  ...
  const columns = [
    ...
    { key: 'slug', label: 'Slug', filterable: true },
    {
      ...
      filterable: true,
      render: (item) => item.parent ? item.parent.name : '-'
    },
    {
      key: 'products',
      label: 'Products',
  ...
  const handleBulkAction = async (action, selectedIds) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to ${action} categories`);
      }

      showNotification(`Categories ${action}d successfully`);
      fetchCategories();
    } catch (error) {
  ...
  const actions = (category) => (
    <div className="flex justify-end space-x-2">
      <button
        onClick={(e) => {
          e.stopPropagation();
          handleAction(category.status === 'active' ? 'deactivate' : 'activate', category.id);
        }}
